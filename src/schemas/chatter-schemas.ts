...
export const CreateChatterRequestSchema = z.object({
	...
	created_at: z.string().datetime().optional().describe("Override creation timestamp (ISO8601 format with timezone)"),
...
export const WeatherSummarySchema = z.object({
	...
	precip_type: z.string().optional().describe("Precipitation type (RAIN, SNOW, etc.)"),
	precip_quantity_in: z.number().optional().describe("Precipitation quantity in inches"),
	cloud_pct: z.number().optional(),
	visibility_miles: z.number().optional(),
	uv_index: z.number().optional(),
	dewpoint_f: z.number().optional().describe("Dew point temperature (Fahrenheit)"),
	heat_index_f: z.number().optional().describe("Heat index (Fahrenheit)"),
	wind_chill_f: z.number().optional().describe("Wind chill temperature (Fahrenheit)"),
	wet_bulb_temp_f: z.number().optional().describe("Wet bulb temperature (Fahrenheit)"),
	thunderstorm_prob_pct: z.number().optional().describe("Thunderstorm probability (percent)"),
	ice_thickness_in: z.number().optional().describe("Ice thickness (inches)"),
